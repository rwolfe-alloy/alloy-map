...
bench = json.dumps(json.load(open("alloy_benchmarks.json")), separators=(",", ":")) if os.path.exists("alloy_benchmarks.json") else "{}"
...
html, n9 = re.subn(r"const BENCHMARKS = \{.*?\};", lambda m: f"const BENCHMARKS = {bench};", html, count=1)
html, n10 = re.subn(r"const PIPELINE = \{.*?\};",   lambda m: f"const PIPELINE = {pipe};",   html, count=1)

if min(n1, n2, n3, n4, n5, n6, n7, n8, n9, n10) != 1:
    raise SystemExit(f"!! Replacement failed (LOCATIONS={n1}, WHITESPACE={n2}, SBA_LOANS={n3}, ITEM19={n4}, CHURN={n5}, TRENDS={n6}, DEMOG={n7}, COMPETITORS={n8}, BENCHMARKS={n9}, PIPELINE={n10}). index.html not modified.")
